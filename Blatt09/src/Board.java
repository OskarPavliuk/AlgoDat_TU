...
public class Board {
    ...
    public int manhattan() {
        ...
        return IntStream.range(0, field.length)
                .boxed()
                .flatMap(y -> IntStream.range(0, field[0].length)
                        .mapToObj(x -> new int[]{x, y}))
                .filter(pos -> field[pos[1]][pos[0]] != BLANK)
                .mapToInt(pos -> {
                    int currentpostion = field[pos[1]][pos[0]];
                    int realpositionforx = (currentpostion - 1) % nx;
                    int realpositionfory = (currentpostion - 1) / nx;
                    return Math.abs(pos[0] - realpositionforx) + Math.abs(pos[1] - realpositionfory);
                })
                .sum();
    }
